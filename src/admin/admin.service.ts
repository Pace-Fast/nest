...
export class AdminService {
  constructor(
    @InjectRepository(Admin) private readonly catRepo: Repository<Admin>, // 使用泛型注入对应类型的存储库实例
  ) {}
   ...
   * @param cat Cat 实体对象
   */
  async createCat(cat: Admin): Promise<Admin> {
    /**
   ...
   * @param cat Cat 实体对象
   */
  // async updateCat(id: number, cat: Admin): Promise<void> {
   ...
   * @param id ID
   */
  async findOneCat(id: number): Promise<Admin> {
   ...
   * @param id ID
   */
  private async findOneById(id: number): Promise<Admin> {
